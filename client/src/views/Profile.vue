...
export default {
    ...
    methods: {
        ...
        async validateEditForm(){
            let update_flag = 0
            // const response = await axios.get('http://localhost:5000/api/v1/search/slugName'+'?s='+'HLEO');
            // console.log(response)

            if (this.tempAccount.name != this.account.name){
                if (!this.tempAccount.name){
                    const response = await axios.get('http://localhost:5000/api/v1/search/accountName'+'?s='+this.tempAccount.name);
